# ВНИМАНИЕ!
# Этот код требует наличия модуля bs4
...
def open_link_2(short_link):
    ...
    try:
        ...
        surnames = soup.findAll(attrs={"class": "surname"})
        initials = soup.findAll(attrs={"class": "given-names"})
        if len(surnames) > 0:
            for i in range(0, len(surnames)):
                if i < len(initials):
                    x = initials[i].get_text() + ' ' + surnames[i].get_text()
                else:
                    x = surnames[i].get_text()
                x = x.lower()
                references.append(x)
        else:
...
def create_data(references, authors):
    ...
    with open('output_data_cambr.csv', 'a', encoding='utf-8') as f:
        for author in authors:
            x = str(author)
            for reference in references:  # ВНИМАНИЕ тут ограничение на 10 первых референтов из списка
                y = ';' + str(reference)
                x += y
            x += '\n'
            f.write(x)
...
def get_output_data():
    ...
    with open('all_camb_articles.txt', 'r', encoding='utf-8') as f:  # delete "test"
        ...
        # print(len(lines))
        for link in lines[1:]:
            # ВНИМАНИЕ тут ограничение на 10 статей из списка, чтобы обработать все иcпользуйте [1:]
