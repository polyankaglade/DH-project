...
def create_datafile():
    with open('output_data_dialogue.csv','w',encoding='utf-8') as f:
        f.write('')
    print('Создан файл для выходных данных: output_data_dialogue.csv.')

# собирает связку автор + все референты
# записывает в csv файл построчно через разделитель ';' 
def create_data(references,authors):
    # c = Counter(references).most_common()
    # [('ref1',5),('ref2',4),('ref3',3)]
    with open('output_data_dialogue.csv','a',encoding='utf-8') as f:
